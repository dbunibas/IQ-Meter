...
public class FxRecordedQualityChart {
...
//    private ObservableList<XYChart.Series<Number, Number>> seriesOs;
    ...
    FxRecordedQualityChart(JFXPanel fxPanel) {
        ...
        for (MappingTool tool : scenario.getToolsList()) {
            ...
            if (tool.getNumberExecutions() > 0) {
                ...
                if (!mapColor.containsKey(tool.getName())) {
                    ...
                    if (logger.isTraceEnabled()) logger.trace("map color put" + tool.getName() + " value " + mapColor.get(tool.getName()));
                }
                ...
                for (MappingExecution mappingTask : listExecution) {
                    ...
                    if (mappingTask.getEffortRecording() != null) {
                        if (logger.isTraceEnabled()) logger.trace("Updating effort:" + mappingTask.getEffortRecording());
                        effort = mappingTask.getEffortRecording().getTotalInteraction();
                    }
                    series.getData().add(new XYChart.Data(mappingTask.getQuality().getFmeasure() * 100, effort));
                }
